...
from metis.global_init import hermes, vprofile, metisapi, app, GenericModel
...
class User(ModelSchema, UserMixin):
    ...
    def authenticate(cls, accountcode, username, password, ua, ip, device, **kwargs):
        ...
        if res is not None and res.get('status', None) == 'SUCCESS':
            ...
            if len(res_login) > 0:
                ...
                person_metis = self.get_person_metis({
                    'uuid': person_uuid,
    ...
    def get_person_metis(self, args):
        person = metisapi.get('person', args)
        return person
